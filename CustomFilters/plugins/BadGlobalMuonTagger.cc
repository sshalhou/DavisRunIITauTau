...
typedef std::vector<reco::Muon> BadMuonCollection;
    ...
    selectClones_(iConfig.getParameter<bool>("selectClones"))
{
    ...
    //produces<BadMuonCollection>("bad"); /* SZS */
}
...
BadGlobalMuonTagger::filter(edm::Event & iEvent, const edm::EventSetup & iSetup) {
    ...
    // std::auto_ptr<BadMuonCollection> out (new BadMuonCollection); /* SZS */
