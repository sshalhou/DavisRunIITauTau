...
/////////////////////////////
// testing support for multi-trees
// in one job

	std::string treeSequence = std::string(argv[1]);
	std::stringstream treeSequenceStream(treeSequence);

	std::string treeHolder;
	std::vector<std::string> treeVector;

	while(getline(treeSequenceStream, treeHolder, ','))
	{
		std::string treeHolderCopy = treeHolder;
		treeHolderCopy.erase( std::remove(treeHolderCopy.begin(), treeHolderCopy.end(), ' '), treeHolderCopy.end() );
		treeVector.push_back(treeHolderCopy);
	}

std::cout<<" requesting job over the following TTrees .... \n";
for(std::size_t treeIndex = 0; treeIndex<treeVector.size(); ++treeIndex)
{
	std::cout<<" ....... "<<treeVector[treeIndex]<<"\n";
...
for(std::size_t treeIndex = 0; treeIndex<treeVector.size(); ++treeIndex)
{
	...
	TChain * T = new TChain((treeVector[treeIndex]).c_str());
	...
	/* create a string to differentiate output from each TTree iteration */
	std::string outputNameChunk = treeVector[treeIndex];
	std::replace(outputNameChunk.begin(), outputNameChunk.end(), '/', '_');
	outputNameChunk.erase(outputNameChunk.find('_')+1);

	std::cout<<" .............. output will include "<<outputNameChunk<<" in the name \n";
	...
	generateH2TauSyncTree    SYNC(R,operations_map["generateH2TauSyncTree"],outputNameChunk);
