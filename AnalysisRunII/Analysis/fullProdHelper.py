...
topDirName = "/eos/uscms/store/user/gfunk/MayProduction12_1/"
bkgDirLists = [
...
for i in range (0,len(bkgDirLists)):
  ...
  for j in range(0,numberOfLists):
    ...
    print "python operation_submit.py -c -t \"PASSCUTS/FlatTuple\" -i " + str(fileListName) + ".txt" + " -o generateH2TauSyncTree -d " + str(fullDirName) + '\n'
    time.sleep(1)
    subprocess.call("python operation_submit.py -c -t \"PASSCUTS/FlatTuple\" -i " + str(fileListName) + ".txt" + " -o generateH2TauSyncTree -d " + str(fullDirName) + '\n', shell = True)
