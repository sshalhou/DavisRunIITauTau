...
channelMap = { 0:'et', 1:'mt', 2:'tt', 3:'em' }

yieldMat = [[[0 for x in range (7)] for x in range (mass_spec * 2)] for x in range (4)]
infile = open(yields)

bkgtotals = [[0 for x in range (mass_spec)] for x in range (4)]
...
for j in range (0, mass_spec):
  ...
  subprocess.call('combineCards.py eTau=datacard_et_' + massName + '_out.txt muTau=datacard_mt_' + massName + '_out.txt tauTau=datacard_tt_' + massName + '_out.txt eMu=datacard_em_' + massName + '_out.txt > datacard_' + massName + '_Combined_out.txt', shell = True )
